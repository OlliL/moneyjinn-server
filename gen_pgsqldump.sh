...
pg_dump -U postgres --schema=moneyjinn --schema=moneyjinn_hbci --schema-only --clean > ${PROGPATH}/pgsqldump.sql
pg_dump -U postgres --column-inserts --data-only --table=moneyjinn.cmp_data_formats | grep -E '(SELECT|INSERT)' >> ${PROGPATH}/pgsqldump.sql
