...
class CapitalsourceServiceTest extends AbstractTest {
  ...
  void test_userAaddsACapitalsource_userBsameGroupSeessItTooBecauseCacheWasReset() {
    ...
    final GroupID groupId = new GroupID(GroupTransportBuilder.GROUP1_ID);
    // this caches
    final List<Capitalsource> allCapitalsources1 = this.capitalsourceService
        .getAllCapitalsources(user1Id);
    final Capitalsource capitalsource = this.capitalsourceService.getCapitalsourceById(user2Id,
        groupId, new CapitalsourceID(CapitalsourceTransportBuilder.CAPITALSOURCE1_ID));
    final String comment = String.valueOf(System.currentTimeMillis());
    capitalsource.getUser().setId(user2Id);
    capitalsource.setComment(comment);
    // this should also modify the cache of user 1!
    this.capitalsourceService.createCapitalsource(capitalsource);
    final List<Capitalsource> allCapitalsources2 = this.capitalsourceService
        .getAllCapitalsources(user1Id);
